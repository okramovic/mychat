...
//const App = require(__dirname + '/app/components/ii')
...
app.get("/", (req, res) =>{
  console.log(process.cwd(), path.join(__dirname, 'app/index.html'))
...
app.get("/bot", (req, res) =>{
    ...
    res.status(200).end('server-side rendering in progress, bot sleeps')
})
app.get('/pub',(req,res)=>{
    res.sendFile(__dirname + '/app/index.html');
    //res.redirect('/')
...
app.post('/api/roomContent', jsonParser, checkAccess, allowRoomAccess, async function(req, res){
    
    console.log('req body', req.session._id, req.body, req.body.room)
    //console.log('', req.locals.ok)
  
    // get some amount of older messages to send them to user / will be prepended to messages array on client
  
    const data = await getMessagesFromRoom(req.body.room, req.body.lengthOfClientMessages)
...
function getMessagesFromRoom(room, lengthOfClientMessages = 0, toGet = 10){
    return new Promise((resolve, reject)=>{
        ...
        fs.readFile(`rooms/${room}.json`, (err, data)=>{
          if (err) {
            ...
            const start = lengthOfClientMessages + toGet
            data = JSON.parse(data)
            
            //data.chat.splice(0, data.chat.length - lengthOfClientMessages - toGet)
            //data.chat = data.chat.splice(0, toGet)
            //console.log(data.chat.length - lengthOfClientMessages - toGet, data.chat.length - lengthOfClientMessages)
            
            data.chat = data.chat.slice(data.chat.length - lengthOfClientMessages - toGet, data.chat.length - lengthOfClientMessages)
...
// auto delete saved files (in all sentfiles sub-folders) if older than 48 hours
function deleteOldFiles(){
  fs.readdir('./sentfiles', (er, entries)=>{
    if (er) throw er
    else {
      ...
      entries.forEach(entry =>{
          const stats = fs.statSync('./sentfiles/' + entry)
      ...
      dirs.forEach(dir=>{
        ...
        fs.readdir('./sentfiles/' + dir, (er, entries)=>{
          entries.forEach(entry=>{
            
            fs.stat('./sentfiles/' + dir + '/' + entry, (er, stats)=>{
              //console.log('    ', dir, ' / ',entry, stats.birthtime)
              const fileCreated = Date.parse(stats.birthtime)
              const delTime = fileCreated + (48 * 60 * 60 * 1000)
              const isOver2Days = (Date.now() - fileCreated)/(1000*60*60*24) > 2  // how many days is it here, more than 2?
              
              if (isOver2Days){
                  console.log('file "deleted"', './sentfiles/' + dir + '/' + entry)
                  fs.unlink('./sentfiles/' + dir + '/' + entry,(err)=>{
                    if (err) throw err
                  })
              }              
            })
